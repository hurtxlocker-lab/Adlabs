 ...
 * If a seed string is provided, items are shuffled before assignment; otherwise, discovery input order is preserved.
 */
export function assignCreativesToSlots(
  ...
  const candidatePool = seed ? seededShuffle(items, seed) : [...items];
  ...
  for (const slot of orderedSlots) {
    const compatibleCandidates = candidatePool.filter(
      (candidate) =>
...
export function partitionCreativesIntoPlates(
  ...
  while (remainingPool.length > 0 && plateIndex < maxPlates) {
    ...
    const plateSeed = baseSeed ? `${baseSeed}-plate-${plateIndex + 1}` : undefined;
